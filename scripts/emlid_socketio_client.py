...
class EmlidSocketIOParameters:
	...
	def __init__(self, emlid_ip=None, emlid_port=None):
		...
		self.pos_key = 'pos llh single (deg,m) rover'
		self.status_key = 'solution status'
		self.ar_ratio_key = 'ratio for ar validation'
		self.baseline_key = 'baseline length float (m)'
...
class EmlidSocketIOClient(EmlidSocketIOParameters):
	...
	def __init__(self, emlid_ip=None, emlid_port=None, test_routine=False):
		...
		self.gps_pos_publisher = rospy.Publisher("/fix", NavSatFix, queue_size=1)
		self.connected_publisher = rospy.Publisher("/emlid_connected", Bool, queue_size=1)
		self.ar_ratio_publisher = rospy.Publisher("/emlid_ar_ratio", Float32, queue_size=1)
	...
	def on_connect(self):
		...
		self.connected_publisher.publish(True)
		self.msg_num = 0
	...
	def on_reconnect(self):
		print('Reconnected to Emlid SocketIO server.')
		# self.is_connected = True
		self.msg_num = 0
		self.connected_publisher.publish(True)
	...
	def on_status_broadcast(self, msg):
		...
		print("GPS Position: {}".format(emlid_data[self.pos_key]))
		print("GPS Status: {}".format(emlid_data[self.status_key]))
		print("GPS AR Validation Ratio: {}".format(emlid_data[self.ar_ratio_key]))
		self.msg_num += 1

		# Checks solution status, publishes true/false to /gps_stop topic: 
		self.check_for_fix(emlid_data[self.status_key])

		# Publishes GPS fix status:
		self.solution_status_publisher.publish(emlid_data[self.status_key])

		# Publishes GPS position as NavSatFix on /fix topic:
		self.publish_emlid_pos(emlid_data[self.pos_key])

		# Publishes GPS ar validation ratio:
		self.publish_ar_ratio(emlid_data[self.ar_ratio_key])

		# Publishes GPS connection status:
		self.connected_publisher.publish(True)



	def publish_ar_ratio(self, ar_ratio):
		"""
		Publishes AR validation ratio on /emlid_ar_ratio topic.
		"""
		try:
			ar_ratio = float(ar_ratio)
		except Exception:
			pass
		self.ar_ratio_publisher.publish(ar_ratio)



	def publish_emlid_pos(self, pos_data):
		"""
		Parses the lat,lon,alt string into a NavSatFix object, 
		and publishes to /fix topic.
		"""

		# Gets LLH values as list (e.g., [lat, lon, alt]):
		emlid_list = pos_data.split(',')

		# Creates NavSatFix object:
		navsat_pos = NavSatFix()
		navsat_pos.latitude = float(emlid_list[0])
		navsat_pos.longitude = float(emlid_list[1])
		navsat_pos.altitude = float(emlid_list[2])
